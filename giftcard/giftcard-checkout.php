...
function woocommerce_ajax_apply_giftcard($giftcard_code)
 {
	...
	$giftcard_number = sanitize_text_field( $_POST['giftcard_code'] );
      if(wpr_get_giftcard_balance( $giftcard_number )!='Invalid')
     {

		if(check_card_number_exist(WC()->session->giftcard_post,$giftcard_number)!='true') 
		{
     		if( wpr_get_giftcard_balance( $giftcard_number ) > 0 ) 
			{
		       wpr_get_giftcard_balance( $giftcard_number );
		       wpr_set_session_data(WC()->session->giftcard_post,$giftcard_number);
...
{
	wc_add_notice( __( 'Gift Card Number is Invalid !', 'rpgiftcards' ), 'error' );  //  You already have a gift card in the cart
}
		wc_print_notices();
...
function subtract_giftcard( $wc_cart ) 
{
 ...
 //  $balance = wpr_get_giftcard_balance( $giftcard_id );
  //$balance=wpr_get_total_giftcard_balance($giftcard_id);

	if ( isset( $giftcard_id ) ) 
{
		$balance = wpr_get_total_giftcard_balance( $giftcard_id );

		$charge_shipping 	= "yes"; //get_option('woocommerce_enable_giftcard_charge_shipping');
		$charge_tax 		= "yes"; //get_option('woocommerce_enable_giftcard_charge_tax');
		$charge_fee 		= "yes";//get_option('woocommerce_enable_giftcard_charge_fee');
...
//echo $giftcardPayment;
//echo "<br/>";
//echo $balance;
//echo $charge_shipping."<br/>".$charge_tax ."<br/>".WC()->session->shipping_tax_total;
//exit;
...
function rpgc_display_giftcard( $order ) {
	...
	$theIDNum =  get_post_meta( $order->id, 'rpgc_gift_card_number', true );
...
function rpgc_update_card( $order_id ) 
{
	...
	global $woocommerce;
 	...
 	if ( $giftCard_id <> '' ) 
	{
		 $newBalance = wpr_get_total_giftcard_balance( $giftCard_id ) - WC()->session->giftcard_payment;
      //  update_post_meta( $order_id, 'rpgc_check_code',0);
      ...
      $deduct_amount=WC()->session->giftcard_payment;
	  $transaction_id=date('YmdHis');
	 
    if(wpr_update_stm_giftcard_balance($giftCard_id, $newBalance,$deduct_amount,$transaction_id,$order_id))
	{
	    update_post_meta( $order_id, 'rpgc_transaction_id', $transaction_id);
		$giftCard_IDs = get_post_meta ( $giftCard_id, 'wpr_existingOrders_id', true );
		$giftCard_IDs[] = $order_id;
     //    wpr_update_stm_giftcard_balance($giftCard_id, $newBalance,$deduct_amount,$transaction_id,$order_id);
		...
		update_post_meta( $order_id, 'rpgc_gift_card_number', json_encode($giftCard_id) );
	...
	{
	echo '<h1>There are some problem on smart Trnsaction services </h1>';
	}
	}
