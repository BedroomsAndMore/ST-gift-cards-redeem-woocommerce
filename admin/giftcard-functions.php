...
{	
	$giftCard_id = get_post_meta( $order_id, 'rpgc_gift_card_number', true );
	$giftCard_refunded = get_post_meta( $order_id, 'rpgc_refunded', true );
    $giftcard_balance=0;
	if ( $giftCard_id  && ! ( $giftCard_refunded == 'yes' ) )
	{
	    $refunded_trans= get_post_meta( $order_id, 'rpgc_transaction_id');
		$Auth_Reference_array= get_post_meta( $order_id, 'rpgc_Auth_Reference');
		$Auth_Reference_array=$Auth_Reference_array[0];
		$assoc_array=json_decode($Auth_Reference_array);
		foreach($assoc_array as $single_row)
		{
		$mixed_array=explode('_',$single_row);
		$Auth_Reference=$mixed_array[1];
		$card_number=$mixed_array[0];
		...
		<Auth_Reference>'.$Auth_Reference.'</Auth_Reference>
		<Merchant_Number>'.$merchant_id.'</Merchant_Number>
		<Terminal_ID>'.$terminal_id.'</Terminal_ID>
		<Trans_Type>L</Trans_Type>
		<POS_Entry_Mode>M</POS_Entry_Mode>
		<Transaction_ID>'.date('YmdHis').'</Transaction_ID>
		</Request>';
		$oldBalance=wpr_get_giftcard_balance($card_number);
		$response=socket_programming($refund);
		if($response['Response_Code']=='00')
		{
		$balance= (float)  $response_array['Amount_Balance'];
		$refundAmount = get_post_meta( $order_id, 'rpgc_payment', true );
		$cgiftcard_balance = (float) $oldBalance + (float) $refundAmount;
        $giftcard_balance=$giftcard_balance+$cgiftcard_balance;
		$array_pay=array('giftcard_balance'=>$giftcard_balance,
		'balance'=>$balance,
		...
		$refundAmount = get_post_meta( $order_id, 'rpgc_payment', true );
		 $mygiftcard_balance = get_post_meta( $order_id, 'rpgc_balance', true );
		$tot_amount=$mygiftcard_balance+$refundAmount;
	    update_post_meta($order_id, 'rpgc_balance', $tot_amount ); // Update balance of Giftcard
